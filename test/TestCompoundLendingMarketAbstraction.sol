...
contract TestCompoundLendingMarketAbstraction is Env, GCFormulae, CompoundLendingMarketAbstraction(Addresses.cDAI)
{
	...
	function test02() public
	{
		...
		uint256 _amountcDAI = _calcCostFromUnderlyingCost(100e18, _getExchangeRate(Addresses.cDAI));
	...
	function test03() public
	{
		...
		uint256 _amountDAI = _calcUnderlyingCostFromCost(5000e8, _getExchangeRate(Addresses.cDAI));
		uint256 _amountcDAI = _calcCostFromUnderlyingCost(_amountDAI, _getExchangeRate(Addresses.cDAI));
	...
	function test04() public
	{
		...
		Assert.equal(_getBalance(Addresses.cUSDC), 0e8, "cUSDC balance must be 0e8");
		Assert.equal(_getBorrowAmount(Addresses.cUSDC), 50e6, "USDC borrow balance must be 50e6");

		_safeRepay(Addresses.cUSDC, 50e6);

		Assert.equal(_getBalance(Addresses.USDC), 0e6, "USDC balance must be 0e6");
		Assert.equal(_getBalance(Addresses.cUSDC), 0e8, "cUSDC balance must be 0e8");
		Assert.equal(_getBorrowAmount(Addresses.cUSDC), 0e6, "USDC borrow balance must be 0e6");
