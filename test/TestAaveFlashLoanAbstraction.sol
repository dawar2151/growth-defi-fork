...
contract TestAaveFlashLoanAbstraction is Env
{
	function test01() public
	{
		...
		Assert.isAbove(_liquidityAmount2, 0e18, "DAI liquidity must be above 0e18");

		uint256 _liquidityAmount3 = AaveFlashLoanAbstraction._getFlashLoanLiquidity(address(0));
		Assert.equal(_liquidityAmount3, 0e18, "invalid token liquidity must be 0e18");
	}
}
